...
func ParseExchangeInfoResponseTable(symbol Symbol) Table.ExchangeInfoSymbol {
	...
	for _, fi := range symbol.Filters {
		if fi.FilterType == "MIN_NOTIONAL" {
			ei.FMinNotional = CoreUtils.StringToFloat64(fi.Notional)
		} else if fi.FilterType == "PRICE_FILTER" {
			ei.FMaxPrice = CoreUtils.StringToFloat64(fi.MaxPrice)
			ei.FMinPrice = CoreUtils.StringToFloat64(fi.MinPrice)
			ei.FPriceStep = CoreUtils.StringToFloat64(fi.TickSize)
		} else if fi.FilterType == "LOT_SIZE" {
			ei.FMinQty = CoreUtils.StringToFloat64(fi.MinQty)
			ei.FMaxQty = CoreUtils.StringToFloat64(fi.MaxQty)
			ei.FQtyStep = CoreUtils.StringToFloat64(fi.StepSize)
		}
	}
	return ei
}
