...
func NewBinaHttpUtilsFromConfig(config BinaHttpUtilsConfig) BinaHttpUtils {
	return NewBinaHttpUtils(
		HttpUtils.SetProxy(config.Proxy.Porto, config.Proxy.Host, config.Proxy.Port),
		config.Key.ApiKey,
...
func NewBinaHttpUtils(option HttpUtils.RequestOption, ApiKey, SecretKey, BaseUrl string, log bool) BinaHttpUtils {
	return BinaHttpUtils{
		request:   HttpUtils.CreateRequest(option),
		SecretKey: SecretKey,
		ApiKey:    ApiKey,
		BaseUrl:   BaseUrl,
		Log:       log,
...
func (binaHttpUtils *BinaHttpUtils) Request(api Api) (string, error) {
	...
	if api.Sign {
		...
		urlPart = append(urlPart, binaHttpUtils.sign(queryString))
	...
	if api.HttpMethod == HttpUtils.HttpPost {
	...
	} else if api.HttpMethod == HttpUtils.HttpGet {
